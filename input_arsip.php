...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  $no_isi           = trim($_POST["no_isi_berkas"] ?? "");
  $pencipta         = trim($_POST["pencipta_arsip"] ?? "");
  $no_surat         = trim($_POST["no_surat"] ?? "");
  $tanggal          = trim($_POST["tanggal_surat"] ?? ""); // YYYY-MM-DD
  $uraian           = trim($_POST["uraian_informasi"] ?? "");

  $jumlah           = trim($_POST["jumlah"] ?? "");
  $tingkat          = trim($_POST["tingkat_perkembangan"] ?? "");
  $lokasi           = trim($_POST["lokasi_simpan"] ?? "");
  $keterangan       = trim($_POST["keterangan"] ?? "");

  // validasi wajib
  if ($no_berkas === "" || $kode_klasifikasi === "" || $nama_berkas === "") {
    $error = "Field wajib (*) masih kosong.";
  } elseif (!ctype_digit($no_berkas)) {
    $error = "NO BERKAS harus angka.";
  } elseif ($no_isi !== "" && !ctype_digit($no_isi)) {
    $error = "NO. ISI BERKAS harus angka (kalau diisi).";
  } elseif ($tanggal !== "" && !preg_match('/^\d{4}-\d{2}-\d{2}$/', $tanggal)) {
    $error = "Tanggal harus format YYYY-MM-DD (contoh: 2019-12-01).";
  } else {
    // convert kosong -> NULL utk integer/date
    $no_isi  = ($no_isi === "") ? null : (int)$no_isi;
    $tanggal = ($tanggal === "") ? null : $tanggal;

    try {
      $stmt = $pdo->prepare("
        INSERT INTO arsip_dimusnahkan
          (no_berkas, kode_klasifikasi, nama_berkas, no_isi, pencipta, no_surat, uraian, tanggal, jumlah, tingkat, lokasi, keterangan, created_at)
        VALUES
          (:no_berkas, :kode, :nama, :no_isi, :pencipta, :no_surat, :uraian, :tanggal, :jumlah, :tingkat, :lokasi, :keterangan, NOW())
      ");

      $stmt->execute([
        ":no_berkas"  => (int)$no_berkas,
      ...
      header("Location: tabel_arsip.php?success=1");
      exit;
